...
class IlocCodec:
    ...
    def decode(bytesData):
        ...
        a_hex = hexlify(struct.pack('>I', a)).decode()
        r_value_hor = x
        r_value_ver = y
        r_value_idx = z
        unknown = a
        ...
        val = f'Location: ({str(r_value_hor)}, {str(r_value_ver)}), '\
              f'Selected Index: {str(r_value_idx)}, Unknown: {unknown}/{a_hex}'
        return val
...
class DilcCodec:
    ...
    def decode(bytesData):
        """
        h_array:
        GridQuadrant - short?: Indicates the quadrant on the screen the icon is located.
        1=top right, 2=bottom right, 3=bottom left, 4=top left
        Unk2 - short?: Unknown. Values other than 0 have been observed
        ...
        GridIconPosFromLeft - position from left
        GridIconPosFromTop - position from top
        """
        #if isinstance(bytesData, bytearray):
        ...
        if int(h_str[16:24], 16) > 65535:
            h_pos = f"IconPosFromRight: {str(baseline - int(h_str[16:24], 16))}"
        else:
            h_pos = f"IconPosFromLeft: {str(int(h_str[16:24], 16))}"

        if int(h_str[24:32], 16) > 65535:
            v_pos = f"IconPosFromBottom: {str(baseline - int(h_str[24:32], 16))}"
        else:
            v_pos = f"IconPosFromTop: {str(int(h_str[24:32], 16))}"
        h_array = (
            f"Unk1: {h_str[:8].decode()}",
            f"GridQuadrant: {str(int(h_str[8:12], 16))}",
            f"Unk2: {h_str[12:16].decode()}",
            h_pos,
            v_pos,
            f"GridIconPosFromLeft: {str(baseline - int(h_str[32:40], 16))}",
            f"GridIconPosFromTop: {str(baseline - int(h_str[40:48], 16))}",
...
class DSStore:
        ...
        with self._get_block(node) as block:
            ...
            if next_node:
                for _ in range(count):
                    ...
                    chk = (e.filename.encode('ascii', 'replace')
                           + str(e.type).encode()
                           + e.code
                           + self.src_name.encode('ascii', 'replace')
                           + hexlify(str(e.value).encode()))
                    ...
                    if e_hash not in self.dict_list:
                        ...
                        self.entries[e_hash] = e
                        self.entries[e_hash].node = (self.dict_list[e_hash].split('unallocated')[1]
                                                     + f'hello, reallocated in {node}')
                ...
                for _ in range(count):
                    ...
                    if e_hash not in self.dict_list:
                        ...
                        self.entries[e_hash] = e
                        self.entries[e_hash].node = (self.dict_list[e_hash].split('unallocated')[1]
                                                     + f'unallocated, reallocated in {node}')
                        self.dict_list[e_hash] = self.dict_list[e_hash] + f', reallocated in {node}'
                    else:
                        sys.exit()
                if self.entries:
    ...
    def read_slack(self, slack, node):
        slack = (unhexlify(slack)).decode('ISO-8859-1')
        search_exp = '('
